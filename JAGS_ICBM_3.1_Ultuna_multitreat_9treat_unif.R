model{
  ...
  for(j in 1:J_Ultuna)
  {
    ...
    for (i in 1:(N_Ultuna)){
      ...
      I_R_cereals_Ultuna[j,i]      <-  (1+exudates_coeff)*0.7*C_percent*((alpha+Yields_cereals_Ultuna[j,i])*(1/(SR_cereals_ult)))
      I_R_root_crops_Ultuna[j,i]   <-  (1+exudates_coeff)*0.7*0.32*C_percent*((alpha+Yields_root_crops_Ultuna[j,i])*(1/(SR_root_crops_ult)))#SR_root_crops_ult))
      I_R_oilseeds_Ultuna[j,i]     <-  (1+exudates_coeff)*0.7*C_percent*((alpha+Yields_oilseeds_Ultuna[j,i])*(1/(SR_oilseeds_ult)))#SR_oilseeds_ult))
      I_R_maize_Ultuna[j,i]        <-  (1+exudates_coeff)*0.7*C_percent*((alpha_maize+Yields_maize_Ultuna[j,i])*(1/(SR_maize_ult)))#SR_maize_ult))
      ...
      O_Ultuna[j,i+1]   	<-  (O_Ultuna[j,i]-flux_sum_Ultuna[j,i])*exp(-k2_treat[j]*re_Ultuna[j,i]) + flux_sum_Ultuna[j,i]*exp(-k1_ult*re_Ultuna[j,i])
  ...
  k1_ult    ~ dunif(0.8-0.8*limits_k, 0.8+0.8*limits_k)
  k2_ult    ~ dunif(0.00605-0.00605*limits_k,0.00605+0.00605*limits_k)
  ...
  #root/shoot ratios priors
  SR_cereals_ult    ~ dunif(11-11*limit_SR,11+11*limit_SR)
